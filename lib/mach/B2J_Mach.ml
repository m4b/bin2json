...
let to_json binary config =
  ...
  (* 
  let symbol_table = SymbolTable.to_json (mach.Mach.nlist |> sort)in
  let relocs = Reloc.to_json mach.Mach.relocations in
  let slide_sectors = Mach.ProgramHeader.get_slide_sectors mach.Mach.program_headers
